...
function getTimelineValue(
  ...
  // Fallback: use date column with 1-day duration
  if (dateColumn) {
    const cv = item.columnValues?.find(
...
export function TimelineView({
  ...
  // View range based on scale
  const viewStart = startOfMonth(viewDate);
  const viewEnd =
    scale === 'weeks'
      ? addDays(viewStart, 42) // 6 weeks
      : addDays(endOfMonth(addMonths(viewStart, 2)), 0); // 3 months
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      title={`${tItem.item.name}: ${format(
                        tItem.start,
                        'MMM d'
                      )} - ${format(tItem.end, 'MMM d')}`}
                    >
