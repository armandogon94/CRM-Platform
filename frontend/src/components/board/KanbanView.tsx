...
export function KanbanView({
  ...
  const statusOptions =
    (statusColumn?.config as { options?: { label: string; color: string }[] })
      ?.options || [];

  // Key columns to show on cards (non-status, non-long_text, first 3)
  ...
  // Uncategorized lane for items without a status
