...
export function LoginPage() {
  const { login } = useAuth();
  const [email, setEmail] = useState('admin@jurispath.com');
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-white/10 rounded-2xl mb-4 backdrop-blur-sm">
            <Scale size={32} className="text-brand-300" />
          </div>
          <h1 className="text-3xl font-bold text-white">JurisPath</h1>
          <p className="text-brand-300 mt-2">Legal Case & Client Management</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="admin@jurispath.com"
                required
              />
            </div>
            <div>
          ...
          <div className="mt-6 p-3 bg-brand-50 rounded-lg">
            ...
            <p className="text-xs text-brand-600">
              Email: admin@jurispath.com | Password: demo123
            </p>
          </div>
        </div>
      </div>
