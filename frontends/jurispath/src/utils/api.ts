...
let authToken: string | null = localStorage.getItem('jurispath_token');

export function setAuthToken(token: string | null) {
  authToken = token;
  if (token) {
    localStorage.setItem('jurispath_token', token);
  } else {
    localStorage.removeItem('jurispath_token');
  }
}
...
export const api = {
  ...
  // Slice 20 C3: wires to the flat DELETE /items/:id shim added in A2.5.
  deleteItem: (id: number) =>
    request<null>(`/items/${id}`, { method: 'DELETE' }),

  // Slice 20 C4: wires to the flat POST /boards shim added in A2.5.
  createBoard: (data: {
    name: string;
