...
function AppContent() {
  ...
  // Slice 20.5 B — currentBoard / currentItems / boardLoading state and the
  // Socket.io setup that mirrored CRUD echoes into them are now owned by
  // the shared useBoard hook (consumed inside <BoardPage>). App-level
  // WebSocket listeners for the active board would have duplicated the
  // hook's internal subscription. allItems for the OverviewDashboard KPI
  // tiles is still loaded here because it spans every board and is
  // outside the single-board useBoard scope.
  ...
  const renderContent = () => {
      ...
      return (
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-500">
            Manage workflow automations across all TrustGuard boards
          </p>
    ...
    // key prop forces useBoard to re-mount with a fresh boardId — without
    // it React would reuse the same hook instance across board switches
    // and the loading state would never re-flash.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-500">Loading TrustGuard...</p>
        </div>
      </div>
    );
  }
