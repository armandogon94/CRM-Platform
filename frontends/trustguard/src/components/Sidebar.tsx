import { LayoutDashboard, ShieldAlert, FileText, ClipboardCheck, Zap, Settings, LogOut } from 'lucide-react';
...
const boardIcons: Record<string, typeof ShieldAlert> = {
  'Claims Pipeline': ShieldAlert,
    ...
    <aside className="w-64 bg-brand-800 text-white flex flex-col h-screen fixed left-0 top-0">
      ...
      <div className="p-5 border-b border-brand-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-lg leading-tight">TrustGuard</h1>
            <p className="text-brand-300 text-xs">Insurance Management</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        ...
        {boards.map((board) => {
          const Icon = boardIcons[board.name] || ClipboardCheck;
          return (
            <button
              key={board.id}
