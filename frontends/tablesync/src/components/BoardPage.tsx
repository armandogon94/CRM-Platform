...
/**
 * TableSync state-based board page (Slice 20B C2 CRUD wiring).
 *
 * Like MedVista (and unlike NovaPay's router-based App), TableSync's
 * parent App.tsx drives board selection via `activeView` state and passes
 * `board` + `items` down as props. This component mirrors them into local
 * state so we can apply optimistic CRUD updates without threading setters
 * through App.
 *
 * C2 migrates from TableSync's local KanbanView/BoardTable fork to the
 * shared BoardView. Three CRUD callbacks are wired through to the
 * TableSync-local REST client (which uses the `tablesync_token` key
 ...
 *
 * TableSync uses status options Requested/Confirmed/Seated/Completed
 * (different from MedVista's pipeline statuses) — the shared
 * normalizeStatusValue helper added in Slice 19.7 handles arbitrary
 * status options, so no special-casing is required here. KanbanView
 * lanes are derived dynamically from the board's status column options.
 */
