...
export function MapView({ board, items }: MapViewProps) {
    ...
    return items
      .map((item) => {
        const locCV = (item.columnValues || []).find(
          (cv) =>
            locationColumnIds.has(cv.columnId) &&
            cv.value &&
  ...
  const center: [number, number] = useMemo(() => {
    if (itemsWithLocations.length === 0) return [40, -74];
    const avgLat =
      itemsWithLocations.reduce((sum, i) => sum + i.location.lat, 0) /
      itemsWithLocations.length;
    const avgLng =
      itemsWithLocations.reduce((sum, i) => sum + i.location.lng, 0) /
      itemsWithLocations.length;
