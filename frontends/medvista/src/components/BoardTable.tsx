...
function renderCellValue(column: Column, value: unknown): React.ReactNode {
  ...
  switch (column.columnType) {
    ...
    case 'number': {
      const config = column.config as { format?: string; suffix?: string };
      ...
      return <span className="font-mono text-sm">{String(value)}{config.suffix ? ` ${config.suffix}` : ''}</span>;
    }
    case 'date': {
            ...
            <tbody>
              {groupItems.map((item) => (
                <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <td className="py-2.5 px-3 text-sm font-medium text-gray-900">{item.name}</td>
                  {columns.map((col) => (
...
export function BoardTable({ board, items }: BoardTableProps) {
  ...
  return (
    <div className="space-y-2">
      {groups.map((group) => (
        <GroupSection key={group.id} group={group} columns={columns} items={items} />
      ))}
    </div>
  );
}
