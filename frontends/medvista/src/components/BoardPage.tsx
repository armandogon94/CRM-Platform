...
/**
 * MedVista state-based board page (Slice 20 C2 CRUD wiring).
 *
 * Unlike NovaPay (router-based), MedVista's parent App.tsx drives board
 ...
 *
 * C2 migrates from MedVista's local KanbanView/BoardTable fork to the
 * shared BoardView. Three CRUD callbacks are wired through to the
 * MedVista-local REST client (which uses the `medvista_token` key
...
export function BoardPage({ board, items: propItems, loading }: BoardPageProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Board Content — Slice 20 C2 uses shared BoardView so CRUD
